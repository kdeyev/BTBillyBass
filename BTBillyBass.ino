    ...
    case 0:  //START & WAITING
      ...
      if (soundVolume > silence) {         //if we detect audio input above the threshold
        if (currentTime > mouthActionTime) {  //and if we haven't yet scheduled a mouth movement
          mouthActionTime = currentTime + mouthMovingTime;
          fishState = 1;  // jump to a talking state
        }
      } else if (currentTime > mouthActionTime + mouthMovingTime) {  //if we're beyond the scheduled talking time, halt the motors
        bodyMotor.halt();
        mouthMotor.halt();
      }
...
void wakeup() {
  bodyMotor.setSpeed(255);                                 // move the body full speed
  bodyMotor.forward();                                     //move the body motor to raise the head
  bodyActionTime = currentTime + floor(random(100, 300));  //schedule action time for 1.5 to 3.0 seconds from current time
}
